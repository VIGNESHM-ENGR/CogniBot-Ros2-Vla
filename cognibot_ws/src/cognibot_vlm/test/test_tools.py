...
def test_validate_converts_numbers_and_checks_enum():
    assert validate_arguments("check_reachability", {"x": 0.3, "y": 0, "z": 0.01}) == {
        "x": 0.3,
        "y": 0.0,
        "z": 0.01,
    }
    assert validate_arguments("fetch_object", {"label": "red cube"}) == {"label": "red cube"}
...
def test_validate_rejects_unknown_missing_extra_and_wrong_types():
    ...
    with pytest.raises(ToolError, match="missing"):
        validate_arguments("place_object", {})
